...
export const useDailyReward = () => {
  ...
  const lastClaim = gameData?.garden?.last_daily_claim_date ?? null;
  const streak = gameData?.garden?.daily_streak ?? 0;
  ...
  const claimMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (result.boost_type) {
        parts.push(`${result.boost_type} ×${result.boost_value} (${result.boost_minutes}min)`);
      }
