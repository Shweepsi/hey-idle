...
export const useAchievements = () => {
  ...
  const baseAchievements: Omit<
    Achievement,
    'id' | 'progress' | 'completed' | 'unlocked_at'
  >[] = [
    {
      name: 'Premier Pas',
  ...
  useEffect(() => {
    ...
    const loadAchievements = async () => {
      try {
        ...
        const mergedAchievements: Achievement[] = baseAchievements.map(
          (baseAchievement) => {
            const saved = savedAchievements?.find(
              (s) => s.achievement_name === baseAchievement.name
            );
  ...
  const checkAchievementProgress = async (_garden: any) => {
    ...
    for (const baseAchievement of baseAchievements) {
      ...
      if (error) {
        console.error(
          `Achievement RPC error (${baseAchievement.name}):`,
          error
        );
        continue;
      }
      ...
      if (result.completed && !result.already_completed) {
        toast.success(
          `🏆 Achievement débloqué : ${baseAchievement.emoji} ${baseAchievement.name}`,
          {
          ...
          prev.map((a) =>
            a.name === baseAchievement.name
              ? {
                  ...a,
                  completed: true,
                  progress: result.progress ?? a.progress,
