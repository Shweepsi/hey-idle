...
export const UpgradesPage = () => {
  ...
  const upgradesByCategory = sequentialUpgrades.reduce(
    (acc, upgrade) => {
  ...
  const getCurrentUpgrade = (upgrades: LevelUpgrade[]) => {
    ...
    const nextUpgrade = sortedUpgrades.find(
      (upgrade) => !isUpgradePurchased(upgrade.id)
    );
  ...
  const getCurrentLevel = (upgrades: LevelUpgrade[]) => {
    ...
    if (categoryKey === 'automatisation') {
      ...
      const robotUpgrades = playerUpgrades.filter(
        (upgrade) => upgrade.level_upgrades?.effect_type === 'robot_level'
      );

      robotUpgrades.forEach((upgrade) => {
        if (
          upgrade.level_upgrades?.effect_value &&
          upgrade.level_upgrades.effect_value > maxLevel
        ) {
  ...
  const getButtonState = (upgrade: LevelUpgrade) => {
    ...
    if (playerLevel < upgrade.level_required)
      return {
        text: 'Verrouillé',
        style: 'bg-gray-400',
      };
    if (coins < upgrade.cost_coins + MINIMUM_COINS_RESERVE)
      return {
        text: 'Pas assez de pièces',
        style: 'bg-red-400',
      };
    if (gems < upgrade.cost_gems)
      return {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {Object.entries(upgradesByCategory).map(([categoryKey, upgrades]) => {
            ...
            return (
              <Card
                key={categoryKey}
                className={`glassmorphism relative overflow-hidden transition-all duration-500 hover:scale-105 ${maxLevel ? 'bg-gradient-to-br from-green-50/80 to-emerald-50/80 border-green-200 shadow-green-100' : canBuy ? 'bg-gradient-to-br from-blue-50/80 to-cyan-50/80 border-blue-200 shadow-blue-100 hover:shadow-blue-200' : isLocked ? 'bg-gradient-to-br from-gray-50/80 to-slate-50/80 border-gray-200 opacity-75' : 'bg-gradient-to-br from-orange-50/80 to-red-50/80 border-orange-200'}`}
              >
                {/* Indicateur de niveau en arrière-plan */}
                <div className="absolute top-2 right-2 z-0">
                  <div
                    className={`text-6xl font-bold opacity-10 ${maxLevel ? 'text-green-600' : 'text-blue-600'}`}
                  >
                ...
                <CardHeader className="relative z-10 pb-2">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    <div className="flex-1 bg-white/50 rounded-full h-2 overflow-hidden">
                      <div
                        className={`h-full transition-all duration-700 ${maxLevel ? 'bg-gradient-to-r from-green-400 to-emerald-500' : 'bg-gradient-to-r from-blue-400 to-cyan-500'}`}
                        style={{
                    ...
                    <Badge
                      variant="outline"
                      className={`text-xs font-bold ${maxLevel ? 'bg-green-100 text-green-700 border-green-300' : 'bg-blue-100 text-blue-700 border-blue-300'}`}
                    >
                  ...
                  {!maxLevel ? (
                    <>
                      ...
                      <div className="flex justify-center gap-3 mb-3">
                        {currentUpgrade.cost_coins > 0 && (
                          <div className="flex items-center gap-1">
                            ...
                            <span
                              className={`font-bold text-sm ${coins >= currentUpgrade.cost_coins + MINIMUM_COINS_RESERVE ? 'text-green-600' : 'text-red-500'}`}
                            >
                        ...
                        {currentUpgrade.cost_gems > 0 && (
                          <div className="flex items-center gap-1">
                            ...
                            <span
                              className={`font-bold text-sm ${gems >= currentUpgrade.cost_gems ? 'text-green-600' : 'text-red-500'}`}
                            >
                      ...
                      <Button
                        ...
                        onClick={() => purchaseUpgrade(currentUpgrade.id)}
                        className={`w-full font-semibold text-xs py-1.5 transition-all duration-300 ${buttonState.style} ${canBuy ? 'hover:scale-105 hover:shadow-lg' : ''}`}
                      >
                        {isPurchasing ? (
                      ...
                      {!isPurchased &&
                        coins <
                          currentUpgrade.cost_coins + MINIMUM_COINS_RESERVE &&
                        coins >= currentUpgrade.cost_coins && (
                    ...
                    <div className="text-center py-3">
                      ...
                      <p className="text-green-600 text-xs">
                        Vous avez débloqué toutes les améliorations de cette
                        catégorie.
                      </p>
                    </div>
                  )}
